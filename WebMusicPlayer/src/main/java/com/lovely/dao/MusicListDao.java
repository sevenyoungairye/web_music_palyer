...
public class MusicListDao {
    ...
    public List<MusicList> queryAllMusicList(MusicList mList, int pageNo, int pageSize) {
        ...
        if (mList != null) {
            // search by case..
            //  and music_list_id = ? and music_list_title like ? and music_list_tag like ?
        ...
        // rowNum ==> pageNo
        parmaList.add((pageNo - 1) * pageSize + 1);
        parmaList.add(pageNo * pageSize);

        List<MusicList> musicLists = jdbcTemplate.query(sql.toString(), parmaList.toArray(),
                new BeanPropertyRowMapper<MusicList>(MusicList.class));
    ...
    public int addMusicList(MusicList musicList) {
         ...
         */
        sql.append("insert into music_list values (seq_music_list.nextval, ?, ?, ");
        sql.append("?, 0, ?, ?, ?, 0)");
    ...
    public List<MusicList> queryRecommendPlayList() {
        ...
        sql.append("(select list.*, rownum rn from music_list list order by music_list_listening_volume desc) ");
        sql.append("where rn between 1 and 6");

        List<MusicList> playList = jdbcTemplate.query(sql.toString(),
                new BeanPropertyRowMapper<>(MusicList.class));
    ...
    public List<MusicSong> getSongInPagePlayList(Long playListId) {
        ...
        sql.append("select * from ( ");
        sql.append("select song.*, rownum rn from music_list list, music_song song, music_list_relation rel ");
        sql.append("where list.music_list_id = rel.music_list_id and song.music_song_id = rel.music_song_id ");
        sql.append("and list.music_list_id = ? ");
        sql.append(")tab1 where rn between 1 and 4");
